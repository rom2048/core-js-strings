...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((sum, cur) => sum + cur.charCodeAt(), 0)
    : 0;
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((sum, cur) => (vowels.has(cur.toLowerCase()) ? sum + 1 : sum), 0);
}
    ...
    .reduce(
      (newStr, char) =>
        alphabet.indexOf(char) === -1
          ? newStr + char
          : newStr + encrypt[alphabet.indexOf(char)],
      ''
    );
}
